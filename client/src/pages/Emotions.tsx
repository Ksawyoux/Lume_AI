...
export default function Emotions() {
  ...
  // Listen for voice analysis errors
  useEffect(() => {
    const handleVoiceAnalysisError = (event: CustomEvent) => {
      setAnalysisError(event.detail.message);
      // Auto-hide the error after 5 seconds
      setTimeout(() => setAnalysisError(null), 5000);
    };
    
    window.addEventListener('voiceAnalysisError', handleVoiceAnalysisError as EventListener);
    
    return () => {
      window.removeEventListener('voiceAnalysisError', handleVoiceAnalysisError as EventListener);
    };
  }, []);
        ...
        <section className="px-4 py-2 mb-4">
          {analysisError && (
            <Alert variant="destructive" className="mb-4">
              ...
              <AlertDescription>
                We couldn't analyze your emotions. Please try again later.
              </AlertDescription>
            </Alert>
          )}
